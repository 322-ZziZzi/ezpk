(() => {
  ...
  const parseTime = value => { if (!value) return null; const d = new Date(value); return Number.isNaN(d.getTime()) ? null : d; };
  const formatStart = date => {
    const parts = new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Seoul',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(date);
    const get = type => parts.find(p => p.type === type)?.value || '';
    return `${get('month')}/${get('day')} ${get('hour')}:${get('minute')} KST`;
  };
  const countdown = ms => { const total=Math.max(0,Math.floor(ms/1000)),days=Math.floor(total/86400),hours=Math.floor((total%86400)/3600),minutes=Math.floor((total%3600)/60),seconds=total%60; return days>0?`${days}D ${pad(hours)}:${pad(minutes)}:${pad(seconds)}`:`${pad(hours)}:${pad(minutes)}:${pad(seconds)}`; };
  ...
  function render(){
    ...
    grid.innerHTML=events.map((event,index)=>{const info=stateOf(event,now);let badge='UPCOMING',timerText='',timerLabel='START IN';if(info.state==='upcoming')timerText=countdown(info.start-now);if(info.state==='live'){badge='LIVE';timerLabel='';timerText='● LIVE';}if(info.state==='finished'){badge='FINISHED';timerLabel='';timerText='FINISHED';}return `<article class="event-card event-${info.state}" style="--event-index:${index}"><div class="event-card-top"><span class="event-number">${pad(index+1)}</span><span class="event-status">${badge}</span></div><h3>${escapeHtml(event.title)}</h3><div class="event-countdown">${timerLabel?`<small>${timerLabel}</small>`:''}<strong>${timerText}</strong></div><time>START · ${formatStart(info.start)}</time></article>`;}).join('');
  }

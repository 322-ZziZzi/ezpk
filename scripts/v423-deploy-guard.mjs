...
if(pkg.version!=='4.2.3')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v423-deploy-guard.mjs')fail('predeploy not v423 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.3'||lock.packages?.['']?.version!=='4.2.3')fail('package-lock root version not 4.2.3');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f);must(t,'/i18n-v414.js?v=4170',`i18n v414 cache token missing ${rel}`);if(t.includes('shared-header.js'))must(t,'shared-header.js?v=4220',`shared header v421 cache token missing ${rel}`);if(!t.includes('data-ezpk-theme-bootstrap'))fail(`theme bootstrap missing ${rel}`)}
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'/text-fit-v419.css?v=4190',`v419 text-fit CSS missing ${rel}`);must(t,'/text-fit-v419.js?v=4190',`v419 text-fit JS missing ${rel}`);}
...
// v423 shared-PC-header readable-fit contract and cache coherency.
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`shared-header cache token missing ${rel}`);must(t,'/header-fit-v423.css?v=4230',`v423 header-fit CSS missing ${rel}`);must(t,'/header-fit-v423.js?v=4230',`v423 header-fit JS missing ${rel}`);if(t.includes('/header-fit-v422.css?v=4220')||t.includes('/header-fit-v422.js?v=4220')||t.includes('/header-fit-v419.css?v=4190')||t.includes('/header-fit-v419.js?v=4190'))fail(`legacy header fitter still active ${rel}`);}
const hfit=read('header-fit-v423.js'),hcss=read('header-fit-v423.css');
for(const token of ["VERSION='423'","const LEVELS=['normal','compact','tight']",'const MIN_VISIBLE_PRIMARY=4','const PRIMARY_FONT_FLOOR_PX=13','const COLLISION_GAP_PX=8','function restorePrimary','function moveLastPrimaryToMore','function horizontalConflict','function collisionCount','function primaryFontFloor','Every pass begins from the full, uncompressed Primary Navigation','directPrimaryLinks(h).length>MIN_VISIBLE_PRIMARY','ezpkHeaderPrimaryMinRequired','ezpkHeaderPrimaryFontFloor','ezpkHeaderPrimaryCollision','ezpk-header-layout-change'])must(hfit,token,'v423 header-fit engine');
for(const token of ['@media(max-width:1199px)','alliance-selector-link{display:none!important','font-size:14px!important','font-size:13.5px!important','desktop-alliance-label-full','desktop-alliance-label-short',"data-ezpk-header-fit-level='compact'","data-ezpk-header-fit-level='tight'",'.account-member-name{','text-overflow:ellipsis!important'])must(hcss,token,'v423 header-fit CSS');
if(/desktop-nav-items>a\{[^}]*font-size:(?:9|10|11|12)(?:\.|px)/.test(hcss))fail('v423 Primary Navigation font below 13px');
if(/#navMoreButton\{[^}]*font-size:(?:9|10|11|12)(?:\.|px)/.test(hcss))fail('v423 More button font below 13px');
if(/grid-template-columns\s*:\s*repeat\(/.test(hcss))fail('v423 header-fit must not alter content card grids');
const sh419=read('shared-header.js');for(const token of ["notifyHeaderLayoutChange('navigation')","notifyHeaderLayoutChange('language')","notifyHeaderLayoutChange('account')","notifyHeaderLayoutChange('responsive-navigation')"])must(sh419,token,'v419 shared-header layout notification');
...
// v421 mobile-only dual-alliance button selector.
const v421Header=read('shared-header.js');
const v421Style=read('style.css');
for(const token of [
  'function allianceChoices()',
  'function mobileAllianceSelectorMarkup',
  'data-alliance-choice=',
  'aria-pressed=',
  ...
  'Cross-alliance authentication is host-scoped',
  'mobileDrawerItems?.addEventListener',
  'ALLIANCE_SWITCH_CONFIRM'
])must(v421Header,token,'v421 mobile alliance selector');
for(const token of [
  '/* v421 — Mobile-only alliance context selector.',
  '.mobile-alliance-selector-group',
  '.mobile-alliance-choice-grid',
  'grid-template-columns:repeat(2,minmax(0,1fr))',
  'min-height:46px',
  'var(--ezpk-accent',
  '.mobile-alliance-choice.is-active',
  '@media(max-width:1199px)'
])must(v421Style,token,'v421 mobile alliance selector CSS');
if(/@media\(min-width:1200px\)[\s\S]{0,500}mobile-alliance-choice/.test(v421Style))fail('v421 mobile alliance selector must not add desktop rules');
if(!v421Header.includes("const enabled = siteContext?.mode === 'DUAL'"))fail('v421 selector must remain DUAL-only');
if(!v421Header.includes("disabled:siteContext?.ezpk2Active === false"))fail('v421 selector must respect inactive EZPK2');
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`v421 shared-header cache token missing ${rel}`)}
for(const f of v420StylePages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'style.css?v=4210',`v421 style cache token missing ${rel}`)}


// v422 Alliance compact/full label contract remains, with v423 readable PC fit ownership.
const v422Header=read('shared-header.js');
for(const token of [
  'desktop-alliance-label-full',
  'desktop-alliance-label-short',
...
])must(v422Header,token,'v422 desktop alliance compact/full label contract');
for(const c of LANGS){if(!new RegExp(`${c==='zh-tw'?"'zh-tw'":c}:\\{[^}]*alliance:`).test(v422Header))fail(`v422 compact alliance label missing ${c}`)}
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`shared-header cache token missing ${rel}`);must(t,'/header-fit-v423.css?v=4230',`v423 header-fit CSS cache missing ${rel}`);must(t,'/header-fit-v423.js?v=4230',`v423 header-fit JS cache missing ${rel}`)}
if(!hfit.includes('visibleDirect')||!hfit.includes('minRequired')||!hfit.includes('fontFloor')||!hfit.includes('collisions'))fail('v423 primary-nav runtime audit fields missing');
if(!hfit.includes('links.length<=minRequired'))fail('v423 minimum-visible-primary relocation guard missing');
if(!hfit.includes('restorePrimary(h);')||!hfit.includes("applyLevel(h,'normal')"))fail('v423 stale fit reset contract missing');

if(failures.length){console.error('EZPK v423 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v423 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

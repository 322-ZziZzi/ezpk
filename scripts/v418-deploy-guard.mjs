...
if(pkg.version!=='4.1.8')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v418-deploy-guard.mjs')fail('predeploy not v418 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.8'||lock.packages?.['']?.version!=='4.1.8')fail('package-lock root version not 4.1.8');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f);must(t,'/i18n-v414.js?v=4170',`i18n v414 cache token missing ${rel}`);if(t.includes('shared-header.js'))must(t,'shared-header.js?v=4170',`shared header v416 cache token missing ${rel}`);if(!t.includes('data-ezpk-theme-bootstrap'))fail(`theme bootstrap missing ${rel}`)}
...
// v418 multilingual responsive text-fit remediation.
const tfCss=read('text-fit-v418.css');
const tfJs=read('text-fit-v418.js');
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'/text-fit-v418.css?v=4180',`v418 text-fit CSS missing ${rel}`);must(t,'/text-fit-v418.js?v=4180',`v418 text-fit JS missing ${rel}`);}
for(const token of ["tile: [","compact: [","scale:.92","scale:.85","MIN_FONT = {tile:12.5, compact:12}","scrollWidth<=el.clientWidth+SAFE_PX","scrollHeight<=el.clientHeight+SAFE_PX","lineCount(el)","ResizeObserver","ezpk-language-change","document.fonts.ready","version:'418'"])must(tfJs,token,'v418 text-fit engine');
for(const token of ['.home-quick-grid a{','white-space:normal!important','[data-ezpk-text-fit="tile"]','[data-ezpk-text-fit="compact"]'])must(tfCss,token,'v418 text-fit CSS');
if(/grid-template-columns\s*:/.test(tfCss))fail('v418 text-fit CSS must not change existing grid columns');
if(tfJs.includes("attributeFilter:['hidden','class','style'"))fail('v418 text-fit observer must not watch its own style mutations');
if(/items:'>/.test(tfJs))fail('v418 text-fit registry contains invalid leading-child selector');
...
must(read('season6','index.html'),'season6.css?v=4180','Season6 v418 CSS cache token');
...
for(const f of ['game/index.html','tank-battle/index.html','missile-defense/index.html','drone-hunter/index.html','treasure-hunter/index.html','zombie-defense/index.html','portal-escape/index.html','hero-merge/index.html'])must(read(...f.split('/')),'game-result-modal.css?v=4180',`${f} v418 result CSS cache token`);

if(failures.length){console.error('EZPK v418 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v418 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

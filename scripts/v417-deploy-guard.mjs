...
if(pkg.version!=='4.1.7')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v417-deploy-guard.mjs')fail('predeploy not v416 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.7'||lock.packages?.['']?.version!=='4.1.7')fail('package-lock root version not 4.1.7');

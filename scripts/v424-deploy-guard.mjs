...
if(pkg.version!=='4.2.4')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v424-deploy-guard.mjs')fail('predeploy not v423 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.4'||lock.packages?.['']?.version!=='4.2.4')fail('package-lock root version not 4.2.3');
...
if(failures.length){console.error('EZPK v424 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v424 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

...
if(pkg.version!=='4.1.0')failures.push(`package version must be 4.1.0, got ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v410-deploy-guard.mjs')failures.push('predeploy must use v410 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.0'||lock.packages?.['']?.version!=='4.1.0')failures.push('package-lock root version must be 4.1.0');
...
must(gateway,'/gateway/gateway.css?v=4100','gateway CSS cache missing');
...
for(const value of [
  'margin:0 auto 16px;padding:0;text-align:center','margin:10px auto 0','.gateway-grid{display:grid;width:100%;padding:0',
...
]) must(gatewayCss,value,'gateway v410 balance/spacing contract missing');
...
if(failures.length){
  console.error('EZPK v410 deployment preflight FAILED.');
  for(const f of failures)console.error(`- ${f}`);
  process.exit(1);
}
console.log('EZPK v410 deployment preflight PASS: Gateway section-padding collision is neutralized, Korean alliance heading is shortened, Super Admin join-code management is host-scoped and safe, /api/db-test redacts sensitive settings, and the 30-migration DB baseline is preserved.');

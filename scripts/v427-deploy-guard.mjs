...
if(pkg.version!=='4.2.7')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v427-deploy-guard.mjs')fail('predeploy not v426 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.7'||lock.packages?.['']?.version!=='4.2.7')fail('package-lock root version not 4.2.6');
...
for(const token of [
  ...
  '../request/request-admin.js?v=4270',
...
])must(adminIndex,token,'v427 admin critical asset wiring');
...
const adminScriptPos=adminIndex.indexOf('admin.js?v=4270');
...
if(!adminIndex.includes('admin.js?v=4270')||!adminIndex.includes('../shared-header.js?v=4270'))fail('v427 admin cache token missing');

if(failures.length){console.error('EZPK v427 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v427 deployment preflight PASS: v425/v424/v421 protected behavior remains intact; administrator authority is single-source through /api/admin/my-permissions, the admin Header no longer depends on public auth bootstrap, and the mobile administrator drawer is independently bound and viewport-contained.');

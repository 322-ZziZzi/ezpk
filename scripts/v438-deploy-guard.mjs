...
if(pkg.version!=='4.3.8')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v438-deploy-guard.mjs')fail('predeploy not v438 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.8'||lock.packages?.['']?.version!=='4.3.8')fail('package-lock root version not 4.3.8');

const config=read('wrangler.jsonc');
for(const v of ['"SITE_MODE": "SINGLE"','"EZPK2_STATUS": "ARCHIVED"','"EZPK1_MIGRATION_INTAKE": "DISABLED"','"EZPK2_MIGRATION_INTAKE": "DISABLED"','"run_worker_first": true','"database_name": "ezpk-members"','"database_id": "aaa29a3a-a221-47e3-a30f-9b4c624dcb56"','"pattern": "ezpk322.com"','"pattern": "ezpk1.ezpk322.com"','"pattern": "ezpk2.ezpk322.com"'])must(config,v,'production config');
if(config.includes('\"binding\": \"EZPK2_DB\"')||config.includes('\"database_name\": \"ezpk2-members\"')||config.includes('7203fea0-0dd3-4332-9c11-44273355a4bb'))fail('v438 production config must not bind EZPK2 D1');
if(pkg.scripts?.['migrate:ezpk2:list']||pkg.scripts?.['migrate:ezpk2:remote'])fail('v438 package scripts must not operate EZPK2 D1');


// v438 guest-first migration entry and migration-cycle reset contract.
...
if(migrationPos<0)fail('v438 home migration entry missing');
if(memberPos<0||migrationPos>memberPos)fail('v438 guest migration entry must be the first home-main content block');
for(const v of ['data-home-migration hidden','shared-migration-entry.js?v=4380','home-v319.js?v=4380'])must(homeIndex,v,'v438 home migration entry');
const homeAuth=read('home-v319.js');
must(homeAuth,"document.querySelectorAll('[data-home-migration]').forEach(function (element) { element.hidden = !anonymous; });",'v438 migration auth visibility');
const resetSql=read('operations','v438_migration_cycle_reset.sql');
for(const v of ['DELETE FROM migration_inquiry_replies;','DELETE FROM migration_inquiries;','DELETE FROM migration_inquiry_sessions;','DELETE FROM migration_applications;','DELETE FROM migration_import_batches;','DELETE FROM migration_rate_limits;',"category = 'migration'","action LIKE 'migration_inquiry_%'",'DELETE FROM sqlite_sequence'])must(resetSql,v,'v438 migration cycle reset');
if(resetSql.includes('DELETE FROM migration_tier_settings'))fail('v438 reset must preserve migration_tier_settings');
const verifySql=read('operations','v438_migration_cycle_verify.sql');
for(const v of ['migration_applications','migration_import_batches','migration_rate_limits','migration_inquiry_sessions','migration_inquiries','migration_inquiry_replies','migration_related_admin_logs','migration_tier_settings_preserved'])must(verifySql,v,'v438 migration cycle verification');
for(const key of ['migration:cycle:verify','migration:cycle:reset'])if(!pkg.scripts?.[key])fail(`v438 package script missing ${key}`);
...
// v438 single-alliance navigation contract replaces the retired v421/v422 alliance selector.
const v438Header=read('shared-header.js');
const v438Style=read('style.css');
if(v438Header.includes('id="allianceSelectorLink"'))fail('v438 desktop alliance selector link must be removed');
if(v438Header.includes('data-alliance-choice=')||v438Header.includes('function allianceChoices()')||v438Header.includes('function switchAllianceFromMobile'))fail('v438 alliance-switch implementation must be retired');
for(const token of [
...
])must(v438Header,token,'v438 single-alliance shared header');
...
const v438Worker=read('worker.js');
for(const token of [
...
])must(v438Worker,token,'v438 worker single-alliance routing');

if(v438Worker.includes('NICKNAME_TAKEN_OTHER_ALLIANCE')||v438Worker.includes('peerNicknameDuplicate')||v438Worker.includes('env.EZPK2_DB'))fail('v438 backend must not perform cross-alliance nickname or EZPK2 DB operations');
const v438Home=read('index.html');
for(const token of [
  'id="homeMigrationEntry"',
  'class="immigration-section"',
  'href="https://ezpk1.ezpk322.com/migration/"',
  '/shared-migration-entry.js?v=4380',
  "window.EZPKMigrationEntry?.render('homeMigrationEntry',lang)",
  "window.addEventListener('ezpk-language-change',renderHomeMigration)"
])must(v438Home,token,'v438 EZPK1 home migration restoration');
const migrationEntry=read('shared-migration-entry.js');
for(const code of LANGS){const token=code==='zh-tw'?"'zh-tw':":`${code}:`;must(migrationEntry,token,`v438 migration entry language ${code}`)}
if(/href=[\"'][^\"']*ezpk2\.ezpk322\.com/i.test(v438Home))fail('v438 home must not link to EZPK2');
if(config.includes('"SITE_MODE": "DUAL"')||config.includes('"EZPK2_STATUS": "ACTIVE"'))fail('v438 production config regressed to DUAL/ACTIVE');
if(pkg.scripts?.predeploy!=='node scripts/v438-deploy-guard.mjs')fail('v438 predeploy script mismatch');

if(failures.length){console.error('EZPK v438 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v438 deployment preflight PASS: SINGLE mode is enforced, Gateway/alliance selection are retired, ezpk322.com routes to EZPK1, EZPK2 pages redirect safely while EZPK2 APIs return 410, EZPK2 D1 is not deployment-bound, EZPK1 migration entry is restored, and the v436 Member Management/rank-review baseline remains intact without a new D1 migration.');

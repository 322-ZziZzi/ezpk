...
if(pkg.version!=='4.4.0')fail(`package version ${pkg.version}`);
if(lock.version!=='4.4.0'||lock.packages?.['']?.version!=='4.4.0')fail('package-lock root version not 4.4.0');
if(pkg.scripts?.predeploy!=='node scripts/v440-deploy-guard.mjs')fail('predeploy not v440 guard');
...
for(const token of ['my.css?v=4400','my.js?v=4400','rankHistoryCardV440','promotionQualificationV440','promotionCycleMetaV440','maintenanceCycleMetaV440','maintenanceRecoveryV440'])must(myHtml,token,'v440 My Page HTML/cache');
...
if(failures.length){console.error(`EZPK v440 deployment preflight FAILED (${failures.length})`);for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v440 deployment preflight PASS: Rank Lifecycle Integrity & History is enforced; R1 maintenance is retired; permanent target-rank qualification, fixed-cycle maintenance/recovery separation, immutable history, single-alliance EZPK1 routing, and open migration intake are preserved.');

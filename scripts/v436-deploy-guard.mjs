...
if(pkg.version!=='4.3.6')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v436-deploy-guard.mjs')fail('predeploy not v436 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.6'||lock.packages?.['']?.version!=='4.3.6')fail('package-lock root version not 4.3.6');
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4270',`v421 shared-header cache token missing ${rel}`)}
for(const f of v420StylePages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'style.css?v=4210',`v421 style cache token missing ${rel}`)}
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4270',`shared-header cache token missing ${rel}`);must(t,'/header-fit-v424.css?v=4240',`v424 header-fit CSS cache missing ${rel}`);must(t,'/header-fit-v424.js?v=4240',`v424 header-fit JS cache missing ${rel}`)}
...
if(migrationFiles.length!==31)fail(`v436 migration count ${migrationFiles.length}, expected 31`);
if(migrationFiles.at(-1)!=='0032_v435_rank_review_cycles.sql')fail(`v436 latest migration ${migrationFiles.at(-1)||'none'}, expected 0032_v435_rank_review_cycles.sql`);
if(migrationFiles.some(f=>f.startsWith('0033_')))fail('v436 must not introduce migration 0033');

if(failures.length){console.error('EZPK v436 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v436 deployment preflight PASS: Member Management renders the base list independently, promotion/demotion reviews are lazy-loaded, R1/R2/R3 new-member protection is visible, review evidence is expandable without extra detail API calls, and EZPK1/EZPK2 retain alliance-specific Admin UX/UI while v435 rank-review schema remains unchanged.');

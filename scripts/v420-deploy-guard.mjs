...
if(pkg.version!=='4.2.0')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v420-deploy-guard.mjs')fail('predeploy not v420 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.0'||lock.packages?.['']?.version!=='4.2.0')fail('package-lock root version not 4.2.0');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f);must(t,'/i18n-v414.js?v=4170',`i18n v414 cache token missing ${rel}`);if(t.includes('shared-header.js'))must(t,'shared-header.js?v=4190',`shared header v419 cache token missing ${rel}`);if(!t.includes('data-ezpk-theme-bootstrap'))fail(`theme bootstrap missing ${rel}`)}
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');if(t.includes('style.css'))must(t,'style.css?v=4200',`v420 style cache token missing ${rel}`);}
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4190',`v419 shared-header cache token missing ${rel}`);must(t,'/header-fit-v419.css?v=4190',`v419 header-fit CSS missing ${rel}`);must(t,'/header-fit-v419.js?v=4190',`v419 header-fit JS missing ${rel}`);}
...
for(const f of v420StylePages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\\\','/');must(t,'style.css?v=4200',`v420 style cache token missing ${rel}`)}

if(failures.length){console.error('EZPK v420 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v420 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

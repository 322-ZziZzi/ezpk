...
if(pkg.version!=='4.2.8')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v428-deploy-guard.mjs')fail('predeploy not v428 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.8'||lock.packages?.['']?.version!=='4.2.8')fail('package-lock root version not 4.2.8');
...
if(failures.length){console.error('EZPK v428 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v428 deployment preflight PASS: v427 administrator authority/header behavior remains intact; ordinary member Request Board data is isolated from optional migration-inquiry merge failures; request-manager initialization and cache coherency are guarded.');

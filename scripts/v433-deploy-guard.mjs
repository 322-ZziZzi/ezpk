...
if(pkg.version!=='4.3.3')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v433-deploy-guard.mjs')fail('predeploy not v433 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.3'||lock.packages?.['']?.version!=='4.3.3')fail('package-lock root version not 4.3.3');
...
if(failures.length){console.error('EZPK v433 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v433 deployment preflight PASS: BGB uses D1-backed draft/publish isolation, server-stamped publication metadata, equal Refresh/Save/Publish controls, and preserves v432 member stat presentation plus prior protections.');

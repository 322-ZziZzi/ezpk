...
if(pkg.version!=='4.1.9')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v419-deploy-guard.mjs')fail('predeploy not v418 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.9'||lock.packages?.['']?.version!=='4.1.9')fail('package-lock root version not 4.1.9');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');if(t.includes('style.css'))must(t,'style.css?v=4190',`v419 style cache token missing ${rel}`);}
...
if(failures.length){console.error('EZPK v419 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v419 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

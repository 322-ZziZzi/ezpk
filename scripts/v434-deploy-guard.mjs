...
if(pkg.version!=='4.3.4')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v434-deploy-guard.mjs')fail('predeploy not v434 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.4'||lock.packages?.['']?.version!=='4.3.4')fail('package-lock root version not 4.3.4');
...
for(const token of [
  ...
  'member-manager-v188.js?v=4340',
  'member-manager-v188.css?v=4340',
  'admin.css?v=4340',
...
if(failures.length){console.error('EZPK v434 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v434 deployment preflight PASS: promotion requirements preserve administrator-selected M/G display units with normalized comparison, EZPK2 promotion states use high-contrast semantic colors, and the Admin light theme uses scoped semantic action/status colors while preserving v433 BGB and prior protections.');

...
if(pkg.version!=='4.3.0')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v430-deploy-guard.mjs')fail('predeploy not v430 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.0'||lock.packages?.['']?.version!=='4.3.0')fail('package-lock root version not 4.3.0');
...
if(failures.length){console.error('EZPK v430 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v430 deployment preflight PASS: UID lookup again grants migration-applicant Request Board access, v401 inquiry schemas are supported, and transient session issuance failures no longer erase an existing applicant cookie.');

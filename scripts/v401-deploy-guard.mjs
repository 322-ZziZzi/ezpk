...
if (missing.length) {
  console.error('EZPK v401 deployment preflight FAILED. Missing expected production configuration:');
  for (const value of missing) console.error(`- ${value}`);
  process.exit(1);
}

console.log('EZPK v401 deployment preflight PASS: DUAL routes and both D1 bindings match the resolved production configuration.');

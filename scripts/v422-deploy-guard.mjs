...
if(pkg.version!=='4.2.2')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v422-deploy-guard.mjs')fail('predeploy not v422 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.2'||lock.packages?.['']?.version!=='4.2.2')fail('package-lock root version not 4.2.2');
...
// v419 shared-PC-header compact-fit contract and cache coherency.
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`v421 shared-header cache token missing ${rel}`);must(t,'/header-fit-v422.css?v=4220',`v422 header-fit CSS missing ${rel}`);must(t,'/header-fit-v422.js?v=4220',`v422 header-fit JS missing ${rel}`);}
const hfit=read('header-fit-v422.js'),hcss=read('header-fit-v422.css');
for(const token of ["VERSION='422'","const LEVELS=['normal','compact','tight']",'function primaryLinks','function allPrimaryExposed','ezpkHeaderPrimaryExpected','ezpkHeaderPrimaryVisible','Primary links are never moved to More','ezpk-header-layout-change'])must(hfit,token,'v422 header-fit engine');
for(const token of ['@media(max-width:1199px)','alliance-selector-link{display:none!important','desktop-alliance-label-full','desktop-alliance-label-short',"data-ezpk-header-fit-level='compact'","data-ezpk-header-fit-level='tight'",'overflow:visible!important'])must(hcss,token,'v422 header-fit CSS');
if(hfit.includes('moveLastPrimaryToMore')||hfit.includes('data-v419-overflow-group'))fail('v422 must not move primary navigation into More');
if(/grid-template-columns\s*:\s*repeat\(/.test(hcss))fail('v422 header-fit must not alter content card grids');
...
// v422 PC header regression repair: primary top navigation must remain physically owned by desktopNavItems.
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`v422 shared-header cache token missing ${rel}`);must(t,'/header-fit-v422.css?v=4220',`v422 header-fit CSS cache missing ${rel}`);must(t,'/header-fit-v422.js?v=4220',`v422 header-fit JS cache missing ${rel}`);if(t.includes('/header-fit-v419.css?v=4190')||t.includes('/header-fit-v419.js?v=4190'))fail(`legacy v419 header fitter still active ${rel}`)}
if(!hfit.includes('primaryExpected')||!hfit.includes('primaryVisible')||!hfit.includes('primaryExposed'))fail('v422 primary-nav runtime audit fields missing');
if(hfit.includes('.appendChild(')||hfit.includes('.prepend('))fail('v422 header fitter must not reparent primary nav DOM');

if(failures.length){console.error('EZPK v422 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v422 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

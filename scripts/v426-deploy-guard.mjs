...
if(pkg.version!=='4.2.6')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v426-deploy-guard.mjs')fail('predeploy not v426 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.6'||lock.packages?.['']?.version!=='4.2.6')fail('package-lock root version not 4.2.6');
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`shared-header cache token missing ${rel}`);must(t,'/header-fit-v424.css?v=4240',`v424 header-fit CSS missing ${rel}`);must(t,'/header-fit-v424.js?v=4240',`v424 header-fit JS missing ${rel}`);if(t.includes('/header-fit-v423.css?v=4230')||t.includes('/header-fit-v423.js?v=4230')||t.includes('/header-fit-v422.css?v=4220')||t.includes('/header-fit-v422.js?v=4220')||t.includes('/header-fit-v419.css?v=4190')||t.includes('/header-fit-v419.js?v=4190'))fail(`legacy header fitter still active ${rel}`);}
...
// v422 Alliance compact/full label contract remains, with v424 adaptive PC fit ownership.
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4220',`shared-header cache token missing ${rel}`);must(t,'/header-fit-v424.css?v=4240',`v424 header-fit CSS cache missing ${rel}`);must(t,'/header-fit-v424.js?v=4240',`v424 header-fit JS cache missing ${rel}`)}
...
for(const token of [
  '/i18n-v414.js?v=4260',
  '../shared-header.js?v=4260',
  '../vehicle-power.js?v=4260',
  'admin.js?v=4260',
  '../supabase-config.js?v=4260',
  '../request/request-admin.js?v=4260',
  'migration-manager.js?v=4260',
  'async id="ezpkXlsxLibrary"',
  'critical administrator auth/bootstrap must execute before optional manager/CDN assets'
])must(adminIndex,token,'v426 admin critical asset wiring');
if(/(?:shared-header\.js|admin\.js|style\.css|admin\.css)\?v=(?:4150|4160)/.test(adminIndex))fail('v426 admin page still references stale critical cache tokens');
const adminScriptPos=adminIndex.indexOf('admin.js?v=4260');
const xlsxPos=adminIndex.indexOf('xlsx-js-style@1.2.0');
if(adminScriptPos<0||xlsxPos<0||adminScriptPos>xlsxPos)fail('v426 admin auth script must precede optional XLSX CDN');
for(const token of [
  "version:'426'",
  'adminLoginGateInitialized',
  'adminNavigationInitialized',
  'fetchAdminAuthState(attempts=3)',
...
])must(adminJs,token,'v426 admin auth/bootstrap engine');
for(const token of [
...
])must(adminCss,token,'v426 admin mobile Drawer contract');

if(failures.length){console.error('EZPK v426 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v426 deployment preflight PASS: v425 governance remains intact; administrator authentication bootstraps before optional assets, stale admin cache tokens are eliminated, and the mobile administrator Drawer contract is explicitly restored without changing the 30-migration DB baseline.');

...
if(pkg.version!=='4.2.1')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v421-deploy-guard.mjs')fail('predeploy not v420 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.1'||lock.packages?.['']?.version!=='4.2.1')fail('package-lock root version not 4.2.1');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f);must(t,'/i18n-v414.js?v=4170',`i18n v414 cache token missing ${rel}`);if(t.includes('shared-header.js'))must(t,'shared-header.js?v=4210',`shared header v421 cache token missing ${rel}`);if(!t.includes('data-ezpk-theme-bootstrap'))fail(`theme bootstrap missing ${rel}`)}
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4210',`v421 shared-header cache token missing ${rel}`);must(t,'/header-fit-v419.css?v=4190',`v419 header-fit CSS missing ${rel}`);must(t,'/header-fit-v419.js?v=4190',`v419 header-fit JS missing ${rel}`);}
const hfit=read('header-fit-v419.js'),hcss=read('header-fit-v419.css');
for(const token of ["VERSION='419'","const SAFETY=5","const MAX_PASSES=2","const MIN_FONT=9.5","data-v419-overflow-group","ezpkHeaderFitExpected","ezpkHeaderFitRegistered","ezpkHeaderFitUnresolved","desktop-language-label","moveLastPrimaryToMore","ezpk-header-layout-change"])must(hfit,token,'v419 header-fit engine');
for(const token of ['@media(max-width:1199px)','alliance-selector-link{display:none!important','max-width:144px','text-overflow:clip!important','desktop-language-label','data-v419-overflow-group'])must(hcss,token,'v419 header-fit CSS');
if(/grid-template-columns\s*:\s*repeat\(/.test(hcss))fail('v419 header-fit must not alter content card grids');
...
for(const token of ['function ownedTargets','resetTargets(ownedTargets(h))','if(mo)mo.disconnect()','observeHeader(h)','syncMoreActive(h)'])must(hfit,token,'v419 header-fit lifecycle safety');
...
for(const f of sharedUserPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'shared-header.js?v=4210',`v421 shared-header cache token missing ${rel}`)}
for(const f of v420StylePages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f).replaceAll('\\','/');must(t,'style.css?v=4210',`v421 style cache token missing ${rel}`)}

if(failures.length){console.error('EZPK v421 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v421 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

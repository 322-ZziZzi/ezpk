...
if(pkg.version!=='4.1.1')failures.push(`package version must be 4.1.1, got ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v411-deploy-guard.mjs')failures.push('predeploy must use v411 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.1'||lock.packages?.['']?.version!=='4.1.1')failures.push('package-lock root version must be 4.1.1');
...
must(gateway,'/gateway/gateway.css?v=4110','gateway CSS cache missing');
...
if(failures.length){
  console.error('EZPK v411 deployment preflight FAILED.');
  for(const f of failures)console.error(`- ${f}`);
  process.exit(1);
}
console.log('EZPK v411 deployment preflight PASS: Gateway desktop section rhythm is migration-to-heading 77px, title-to-description 10px, description-to-grid 26px; corrected width alignment and v410 security/join-code contracts are preserved; the 30-migration DB baseline is unchanged.');

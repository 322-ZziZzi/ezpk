...
if (missing.length) {
  console.error('EZPK v405 deployment preflight FAILED. Missing expected production configuration:');
  for (const value of missing) console.error(`- ${value}`);
  process.exit(1);
}

console.log('EZPK v405 deployment preflight PASS: Worker-first host routing, DUAL routes and both D1 bindings match production configuration.');

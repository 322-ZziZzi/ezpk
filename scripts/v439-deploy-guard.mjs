...
if(pkg.version!=='4.3.9')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v439-deploy-guard.mjs')fail('predeploy not v439 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.9'||lock.packages?.['']?.version!=='4.3.9')fail('package-lock root version not 4.3.9');
...
if(config.includes('\"binding\": \"EZPK2_DB\"')||config.includes('\"database_name\": \"ezpk2-members\"')||config.includes('7203fea0-0dd3-4332-9c11-44273355a4bb'))fail('v439 production config must not bind EZPK2 D1');
if(pkg.scripts?.['migrate:ezpk2:list']||pkg.scripts?.['migrate:ezpk2:remote'])fail('v439 package scripts must not operate EZPK2 D1');

const workerV439=read('worker.js');
must(workerV439,'const DEFAULT_EZPK1_MIGRATION_INTAKE = true;','v439 EZPK1 intake default');
const migrationEntryV439=read('shared-migration-entry.js');
for(const token of ["status:'MIGRATION OPEN'","button:'APPLY FOR MIGRATION'","status:'이민 모집 중'","button:'이민 신청하기'"])must(migrationEntryV439,token,'v439 open migration entry');
if(migrationEntryV439.includes('MIGRATION CLOSED')||migrationEntryV439.includes('이민 종료')||migrationEntryV439.includes('이민 신청 기간 종료'))fail('v439 migration entry must remain open for pre-applications');


// v439 guest-first migration entry + always-open EZPK1 pre-application + migration-cycle reset contract.
const homeIndex=read('index.html');
const migrationPos=homeIndex.indexOf('id="homeMigrationEntry"');
const memberPos=homeIndex.indexOf('class="home-member-content"');
if(migrationPos<0)fail('v439 home migration entry missing');
if(memberPos<0||migrationPos>memberPos)fail('v439 guest migration entry must be the first home-main content block');
for(const v of ['data-home-migration hidden','shared-migration-entry.js?v=4390','home-v319.js?v=4390'])must(homeIndex,v,'v439 home migration entry');
const homeAuth=read('home-v319.js');
must(homeAuth,"document.querySelectorAll('[data-home-migration]').forEach(function (element) { element.hidden = !anonymous; });",'v439 migration auth visibility');
const resetSql=read('operations','v438_migration_cycle_reset.sql');
for(const v of ['DELETE FROM migration_inquiry_replies;','DELETE FROM migration_inquiries;','DELETE FROM migration_inquiry_sessions;','DELETE FROM migration_applications;','DELETE FROM migration_import_batches;','DELETE FROM migration_rate_limits;',"category = 'migration'","action LIKE 'migration_inquiry_%'",'DELETE FROM sqlite_sequence'])must(resetSql,v,'v439 migration cycle reset');
if(resetSql.includes('DELETE FROM migration_tier_settings'))fail('v439 reset must preserve migration_tier_settings');
const verifySql=read('operations','v438_migration_cycle_verify.sql');
for(const v of ['migration_applications','migration_import_batches','migration_rate_limits','migration_inquiry_sessions','migration_inquiries','migration_inquiry_replies','migration_related_admin_logs','migration_tier_settings_preserved'])must(verifySql,v,'v439 migration cycle verification');
for(const key of ['migration:cycle:verify','migration:cycle:reset'])if(!pkg.scripts?.[key])fail(`v439 package script missing ${key}`);
...
// v439 single-alliance navigation contract replaces the retired v421/v422 alliance selector.
const v438Header=read('shared-header.js');
const v438Style=read('style.css');
if(v438Header.includes('id="allianceSelectorLink"'))fail('v439 desktop alliance selector link must be removed');
if(v438Header.includes('data-alliance-choice=')||v438Header.includes('function allianceChoices()')||v438Header.includes('function switchAllianceFromMobile'))fail('v439 alliance-switch implementation must be retired');
for(const token of [
...
])must(v438Header,token,'v439 single-alliance shared header');
...
if(migrationFiles.length!==31)fail(`v438 migration count ${migrationFiles.length}, expected 31`);
if(migrationFiles.at(-1)!=='0032_v435_rank_review_cycles.sql')fail(`v438 latest migration ${migrationFiles.at(-1)||'none'}, expected 0032_v435_rank_review_cycles.sql`);
if(migrationFiles.some(f=>f.startsWith('0033_')))fail('v438 must not introduce migration 0033');


// v438 Single Alliance Restoration / Gateway retirement / EZPK2 archive guard.
const v439Worker=read('worker.js');
for(const token of [
...
])must(v439Worker,token,'v439 worker single-alliance routing');

if(v439Worker.includes('NICKNAME_TAKEN_OTHER_ALLIANCE')||v439Worker.includes('peerNicknameDuplicate')||v439Worker.includes('env.EZPK2_DB'))fail('v439 backend must not perform cross-alliance nickname or EZPK2 DB operations');
const v439Home=read('index.html');
for(const token of [
  'id="homeMigrationEntry"',
  'class="immigration-section"',
  'href="https://ezpk1.ezpk322.com/migration/"',
  '/shared-migration-entry.js?v=4390',
  "window.EZPKMigrationEntry?.render('homeMigrationEntry',lang)",
  "window.addEventListener('ezpk-language-change',renderHomeMigration)"
])must(v439Home,token,'v439 EZPK1 home migration restoration');
const migrationEntryTailV439=read('shared-migration-entry.js');
for(const code of LANGS){const token=code==='zh-tw'?"'zh-tw':":`${code}:`;must(migrationEntryTailV439,token,`v439 migration entry language ${code}`)}
if(/href=[\"'][^\"']*ezpk2\.ezpk322\.com/i.test(v439Home))fail('v439 home must not link to EZPK2');
if(config.includes('"SITE_MODE": "DUAL"')||config.includes('"EZPK2_STATUS": "ACTIVE"'))fail('v439 production config regressed to DUAL/ACTIVE');
if(pkg.scripts?.predeploy!=='node scripts/v439-deploy-guard.mjs')fail('v439 predeploy script mismatch');

if(failures.length){console.error('EZPK v439 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v439 deployment preflight PASS: SINGLE mode is enforced; guest-first/signed-in-hidden migration UI is preserved; EZPK1 migration intake remains OPEN for advance applications; previous-cycle reset tooling is retained independently; EZPK2 stays archived; no new D1 migration is required.');

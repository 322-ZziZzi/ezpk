...
if (!fs.existsSync(themeCss)) failures.push('missing ezpk-theme.css');
  ...
  for (const value of ['html[data-site="ezpk2"]','--ezpk-page-bg:#f1f4f8','--ezpk-surface:#ffffff','--ezpk-text-primary:#0b1220']) {
...
for (const file of htmlFiles) {
  ...
  if (!text.includes('/ezpk-theme.css?v=4070')) failures.push(`missing v407 theme stylesheet: ${rel}`);
...
if (!gateway.includes('/gateway/gateway.css?v=4060')) failures.push('gateway neutral CSS cache must remain v4060');
...
for (const file of sharedHeaderPages) { if (!fs.readFileSync(file,'utf8').includes('shared-header.js?v=4070')) failures.push(`shared-header cache version is not v4070: ${path.relative(root,file)}`); }
const bgbHtml = fs.readFileSync(path.join(root,'bgb','index.html'),'utf8');
if (!bgbHtml.includes('bgb.css?v=4060')) failures.push('BGB consolidated CSS cache must remain v4060');


for (const value of [
  '--ezpk-border:#b7c0cc',
  '--ezpk-border-strong:#7e8b9b',
  '--ezpk-cta-primary-bg:#e2a51b',
  '--ezpk-cta-primary-text:#1c1400',
  '--ezpk-cta-secondary-bg:#dfe5ed',
  'High-Contrast Light Theme + Strong CTA Hierarchy'
]) if (!fs.readFileSync(themeCss, 'utf8').includes(value)) failures.push(`v407 contrast/CTA contract missing: ${value}`);
...
if (failures.length) {
  console.error('EZPK v407 deployment preflight FAILED.');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log('EZPK v407 deployment preflight PASS: v406 routing/DB contracts are preserved and EZPK2 high-contrast Light Theme + Strong CTA hierarchy is present across all 27 pages.');

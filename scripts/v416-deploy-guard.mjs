...
if(pkg.version!=='4.1.6')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v416-deploy-guard.mjs')fail('predeploy not v416 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.6'||lock.packages?.['']?.version!=='4.1.6')fail('package-lock root version not 4.1.6');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f);must(t,'/i18n-v414.js?v=4160',`i18n v414 cache token missing ${rel}`);if(t.includes('shared-header.js'))must(t,'shared-header.js?v=4160',`shared header v416 cache token missing ${rel}`);if(!t.includes('data-ezpk-theme-bootstrap'))fail(`theme bootstrap missing ${rel}`)}
...
if(failures.length){console.error('EZPK v416 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v416 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

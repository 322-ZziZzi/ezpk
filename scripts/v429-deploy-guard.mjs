...
if(pkg.version!=='4.2.9')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v429-deploy-guard.mjs')fail('predeploy not v429 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.9'||lock.packages?.['']?.version!=='4.2.9')fail('package-lock root version not 4.2.9');
...
if(failures.length){console.error('EZPK v429 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v429 deployment preflight PASS: v428 admin Request Board isolation remains intact; UID status lookup is authoritative on migration_applications and optional inquiry schema failures cannot suppress the core result.');

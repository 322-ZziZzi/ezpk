...
if(pkg.version!=='4.3.1')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v431-deploy-guard.mjs')fail('predeploy not v431 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.1'||lock.packages?.['']?.version!=='4.3.1')fail('package-lock root version not 4.3.1');
...
if(failures.length){console.error('EZPK v431 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v431 deployment preflight PASS: migration inquiry deletion uses soft delete on current schema and super-admin hard-delete compatibility on v401 schema without exposing a schema-pending error.');

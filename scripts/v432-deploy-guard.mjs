...
if(pkg.version!=='4.3.2')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v432-deploy-guard.mjs')fail('predeploy not v432 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.2'||lock.packages?.['']?.version!=='4.3.2')fail('package-lock root version not 4.3.2');
...
const adminScriptPos=adminIndex.indexOf('admin.js?v=4320');
...
if(!adminIndex.includes('admin.js?v=4320')||!adminIndex.includes('../shared-header.js?v=4270'))fail('v432 admin cache token missing');
...
must(adminIndex,'admin.js?v=4320','v432 admin JS cache token');
must(adminIndex,'../vehicle-power.js?v=4270','v432 existing Vehicle Power authority preserved');

if(failures.length){console.error('EZPK v432 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v432 deployment preflight PASS: Admin BGB member rows display Industry Lv. + Vehicle #1 while existing sorting, BGB assignment/balancing logic, and v431 protections remain intact.');

...
if(pkg.version!=='4.3.5')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v435-deploy-guard.mjs')fail('predeploy not v435 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.3.5'||lock.packages?.['']?.version!=='4.3.5')fail('package-lock root version not 4.3.5');
...
for(const token of [
  ...
  'member-manager-v188.js?v=4350',
  'member-manager-v188.css?v=4350',
  'admin.css?v=4340',
  '/ezpk-theme.css?v=4350'
...
if(failures.length){console.error('EZPK v435 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v435 deployment preflight PASS: rank-review state is persisted, new members receive 10-day demotion protection, promotion uses a 14-day opportunity with HOLD/recovery, maintenance uses 30-day review cycles, and manual rank overrides reset prior cycles while v434 M/G and semantic colors remain preserved.');

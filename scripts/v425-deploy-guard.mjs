...
if(pkg.version!=='4.2.5')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v425-deploy-guard.mjs')fail('predeploy not v425 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.2.5'||lock.packages?.['']?.version!=='4.2.5')fail('package-lock root version not 4.2.5');
...
if(failures.length){console.error('EZPK v425 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v425 deployment preflight PASS: v424 governance remains intact and the shared Mini Games card contract is flow-safe across PC/tablet/mobile, excluded from generic Text-Fit, and covered across all 14 supported locales without changing the 30-migration DB baseline.');

...
if(pkg.version!=='4.1.5')fail(`package version ${pkg.version}`);
if(pkg.scripts?.predeploy!=='node scripts/v415-deploy-guard.mjs')fail('predeploy not v415 guard');
const lock=JSON.parse(read('package-lock.json'));
if(lock.version!=='4.1.5'||lock.packages?.['']?.version!=='4.1.5')fail('package-lock root version not 4.1.5');
...
for(const f of userPages){const t=fs.readFileSync(f,'utf8'),rel=path.relative(root,f);must(t,'/i18n-v414.js?v=4150',`i18n v414 missing ${rel}`);if(!t.includes('data-ezpk-theme-bootstrap'))fail(`theme bootstrap missing ${rel}`)}
...
if(failures.length){console.error('EZPK v415 deployment preflight FAILED.');for(const f of failures)console.error('- '+f);process.exit(1)}
console.log('EZPK v415 deployment preflight PASS: all-14 effective keysets and v415 remediation overlays are complete; known hard-coded runtime i18n leaks are blocked; generated-media/accessibility localization, responsive Gateway contracts, Worker security, and the unchanged 30-migration DB baseline are preserved.');

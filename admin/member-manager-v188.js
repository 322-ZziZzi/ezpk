(() => {
  ...
  let activityConfirmMemberId=null;
  function openActivityConfirm(id){const m=state.candidates.find(x=>x.id===id);if(!m)return;activityConfirmMemberId=id;$('#activityConfirmMemberV369').textContent=m.nickname;['#activityCheckedLoginV369','#activityCheckedEventV369','#activityCheckedAllianceV369'].forEach(s=>$(s).checked=false);$('#activityConfirmMemoV369').value='';$('#activityConfirmStatusV369').textContent='';$('#activityConfirmDialogV369').showModal()}
  async function submitActivityConfirm(e){e.preventDefault();const payload={checkedLogin:$('#activityCheckedLoginV369').checked,checkedEvent:$('#activityCheckedEventV369').checked,checkedAlliance:$('#activityCheckedAllianceV369').checked,memo:$('#activityConfirmMemoV369').value.trim()};if(!payload.checkedLogin&&!payload.checkedEvent&&!payload.checkedAlliance){$('#activityConfirmStatusV369').textContent='한 개 이상의 확인 항목을 선택해 주세요.';return}try{await api(`/api/admin/members/${activityConfirmMemberId}/activity-confirmation`,{method:'POST',body:JSON.stringify(payload)});$('#activityConfirmDialogV369').close();await load(true);renderCandidates()}catch(_){$('#activityConfirmStatusV369').textContent='활동 확인을 저장하지 못했습니다.'}}
  async function revokeActivityConfirm(id){if(!confirm('이 연맹원의 운영진 활동 확인을 취소할까요?'))return;await api(`/api/admin/members/${id}/activity-confirmation`,{method:'DELETE'});await load(true);renderCandidates()}
  ...
  function logLabel(v){return ({admin_permission:"관리자 권한",member:"연맹원",event:"이벤트",vote:"투표",alliance_layout:"연맹 배치",bgb:"BGB",capital_war:"수도전",season:"시즌",request:"요청",account:"계정"})[v]||v||"-"}
  ...
  function init(){
    ...
    $('#activityConfirmFormV369').onsubmit=submitActivityConfirm;$('#activityConfirmCancelV369').onclick=()=>$('#activityConfirmDialogV369').close();

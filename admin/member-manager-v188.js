(() => {
  ...
  function promotionReason(p,a){const status=p?.review?.status,need=Math.max(0,Number(a?.required||2)-Number(a?.completed||0));if(status==='REVIEWABLE')return {cls:'success',text:'승급 활동 조건을 충족하여 지금 검토할 수 있습니다.'};if(status==='HOLD')return {cls:'missing',text:'14일 승급 기회가 종료될 때까지 활동 조건을 충족하지 못했습니다.'};if(status==='WAIT_REENTRY')return {cls:'pending',text:'R1 등급 유지 조건이 아니라 재승급 검토 재개 조건을 확인하고 있습니다.'};if(status==='WAIT_MAINTENANCE')return {cls:'pending',text:'현재 R2 등급 유지 주기를 완료하면 다음 승급 기회가 열립니다.'};return {cls:'progress',text:need?`활동 조건 ${need}개를 추가로 충족하면 승급 검토가 가능합니다.`:'승급 검토 조건을 확인하고 있습니다.'}}
  function demotionReason(s,a){const need=Math.max(0,Number(a?.required||2)-Number(a?.completed||0)),left=Math.max(0,Number(s?.cycle?.totalDays||30)-Number(s?.cycle?.day||0));if(s?.reviewEligible)return {cls:'missing',text:'30일 유지 조건이 미충족되어 강등 검토가 가능합니다.'};if(s?.maintained)return {cls:'success',text:'현재 30일 등급 유지 주기 조건을 충족했습니다.'};return {cls:'progress',text:`유지 조건 ${need}개 부족 · 검토일까지 ${left}일 남음`}}
  function promotionReviewLabel(p){const s=p?.review?.status,day=Number(p?.review?.day||0);if(s==='REVIEWABLE')return {text:'검토 가능',cls:'reviewable'};if(s==='HOLD')return {text:'활동 미달',cls:'hold'};if(s==='WAIT_REENTRY')return {text:'재승급 대기',cls:'waiting'};if(s==='WAIT_MAINTENANCE')return {text:'유지 확인 중',cls:'waiting'};return {text:`${Math.max(1,day)}/14`,cls:'progress'}}
  ...
  function rankHistoryTypeLabel(x){return x.eventType==='PROMOTION'?'승급':x.eventType==='DEMOTION'?'강등':x.eventType==='RESTORE'?'데이터 복원':x.eventType==='CORRECTION'?'기록 보정':'관리자 변경'}
  function rankHistoryReasonLabel(code){return ({PROMOTION_REQUIREMENTS_MET:'승급 조건 충족',MAINTENANCE_ACTIVITY_FAILED:'등급 유지 활동 조건 미달',ADMIN_MANUAL_ADJUSTMENT:'관리자 직접 변경',RANK_DATA_CORRECTION:'등급 데이터 보정',DATA_RESTORE:'데이터 복원'})[code]||code||'-'}
  async function loadRankHistory(){try{const d=await api('/api/admin/rank-change-history?limit=30');state.rankHistory=d.items||[];$('#rankChangeHistoryV371').innerHTML=state.rankHistory.length?state.rankHistory.map(x=>`<article class="rank-history-row"><strong>${esc(x.nickname)} · ${esc(x.fromRank)} → ${esc(x.toRank)}</strong><span>${rankHistoryTypeLabel(x)} · ${date(x.createdAt)} · 처리자 ${esc(x.changedBy||'-')}</span><small>${esc(rankHistoryReasonLabel(x.reasonCode))}${x.internalNote?` · 내부 메모: ${esc(x.internalNote)}`:''}${x.snapshotQuality==='LEGACY_PARTIAL'?' · 기존 기록(상세 판정 일부 없음)':''}</small></article>`).join(''):'<p class="help">처리 이력이 없습니다.</p>'}catch(_){$('#rankChangeHistoryV371').innerHTML='<p class="help">이력을 불러오지 못했습니다.</p>'}}
  ...
  function openDemotionConfirm(id){const m=state.demotionCandidates.find(x=>x.id===id);if(!m)return;demotionMemberId=id;const a=m.maintenance.failedCycleActivity||m.maintenance.reviewActivity||m.maintenance.activity,i=a.items;$('#demotionConfirmMemberV371').textContent=m.nickname;$('#demotionConfirmRouteV371').textContent=`${m.maintenance.currentRank}에서 ${m.maintenance.targetRank}로 변경합니다.`;$('#demotionConfirmActivityV371').innerHTML=`<strong>확정 유지 주기 활동 ${a.completed} / 4</strong><span>${dateOnly(m.maintenance.cycle?.startedOn)} ~ ${dateOnly(m.maintenance.cycle?.dueOn)} · 투표 ${i.vote.count}회 · 사이트 방문 ${i.visit.count}일 · 본인 스펙 ${i.specUpdate.count}회 · 운영진 ${i.adminConfirmation.passed?'확인':'미확인'}</span>`;$('#demotionReasonV371').value='';$('#demotionConfirmStatusV371').textContent='';updateDemotionSubmit();$('#demotionConfirmDialogV371').showModal()}

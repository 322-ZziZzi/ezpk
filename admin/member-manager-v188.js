(() => {
  ...
  async function load(reset=false){
    ...
    try{const c=await api('/api/admin/promotion-candidates');state.candidates=c.items||[];state.stats.promotion=c.counts?.total||0;state.stats.promotionReviewable=c.counts?.reviewable||0}catch(_){state.candidates=[];state.stats.promotion=0;state.stats.promotionReviewable=0}
    try{const d=await api('/api/admin/demotion-candidates');state.demotionCandidates=d.items||[];state.demotionDeferred=d.deferred||[];state.stats.demotion=d.counts?.total||0;state.stats.demotionReviewable=d.counts?.reviewable||0;state.stats.newProtection=d.counts?.newProtection||0}catch(_){state.demotionCandidates=[];state.demotionDeferred=[];state.stats.demotion=0;state.stats.demotionReviewable=0;state.stats.newProtection=0}
    render();
    syncShared();
  }
  ...
  function renderDemotionCandidates(){
    ...
    const row=m=>{const s=m.maintenance,label=s.reviewEligible?{text:'검토 가능',cls:'demotion-reviewable'}:{text:`${Math.max(1,Number(s.cycle?.day||1))}/30`,cls:'progress'};const i=s.activity?.items||{};return `<article class="promotion-candidate demotion-candidate" data-id="${m.id}"><button class="v188-open secondary" type="button"><span class="promotion-candidate-info"><strong>${esc(m.nickname)}</strong><small>${esc(s.currentRank)} → ${esc(s.targetRank)}</small></span></button><span class="review-cycle-badge ${label.cls}">${esc(label.text)}</span><div class="promotion-candidate-actions"><button class="secondary activity-confirm" type="button">${i.adminConfirmation?.passed?'활동 확인 갱신':'활동 확인'}</button>${i.adminConfirmation?.passed?'<button class="secondary activity-revoke" type="button">확인 취소</button>':''}<button class="secondary demotion-exclude" type="button">강등 제외</button><button class="danger demote-member" type="button" ${!s.reviewEligible?'disabled':''}>${esc(s.targetRank)}로 강등</button></div></article>`};

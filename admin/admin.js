...
function serverPartsToIso(date,time){if(!date||!time)return'';return `${date}T${time}:00-02:00`}
...
function renderEvents(){const grid=$('#eventAdminGrid');if(!grid)return;grid.innerHTML=eventsData.events.map((e,i)=>{const sp=isoToServerParts(e.start),ep=isoToServerParts(e.end);return `<article class="event-admin-card ${e.enabled?'enabled':''}" data-event-i="${i}"><div class="event-admin-card-head"><strong>EVENT ${i+1}</strong><label><input data-event-f="enabled" type="checkbox" ${e.enabled?'checked':''}> 활성화</label></div><div class="event-admin-fields"><label>이벤트명<input data-event-f="title" type="text" maxlength="60" value="${esc(e.title)}" placeholder="예: BGB"></label><div class="event-datetime-group"><span>시작 날짜·시간 (ST)</span><div class="event-date-time-row"><input data-event-f="startDate" type="date" value="${sp.date}"><input data-event-f="startTime" type="time" step="60" value="${sp.time}"></div></div><div class="event-datetime-group"><span>종료 날짜·시간 (ST)</span><div class="event-date-time-row"><input data-event-f="endDate" type="date" value="${ep.date}"><input data-event-f="endTime" type="time" step="60" value="${ep.time}"></div></div></div><p class="event-time-note">24시간 형식으로 입력합니다. 자정은 00:00입니다. 연맹원에게는 시작시간만 표시되며 종료시간 이후 자동으로 FINISHED가 됩니다.</p></article>`}).join('');$$('#eventAdminGrid [data-event-f]').forEach(el=>el.onchange=()=>{const card=el.closest('[data-event-i]'),i=Number(card.dataset.eventI),f=el.dataset.eventF;if(f==='enabled')eventsData.events[i].enabled=el.checked;else if(f==='title')eventsData.events[i].title=el.value.trim();else if(f==='startDate')updateEventDateTime(i,'start','date',el.value);else if(f==='startTime')updateEventDateTime(i,'start','time',el.value);else if(f==='endDate')updateEventDateTime(i,'end','date',el.value);else if(f==='endTime')updateEventDateTime(i,'end','time',el.value);card.classList.toggle('enabled',eventsData.events[i].enabled)})}

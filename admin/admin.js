...
function assignMh(sorted){const codes=['M1','M2','H1','H2'],groups=codes.map(code=>({code,names:[],total:0}));for(let bucketIndex=0;bucketIndex<4;bucketIndex++){const bucket=sorted.slice(bucketIndex*5,bucketIndex*5+5);const base=bucket.slice(0,4).sort((a,b)=>b.power-a.power);const order=[...groups].sort((a,b)=>a.total-b.total||codes.indexOf(a.code)-codes.indexOf(b.code));base.forEach((m,i)=>{order[i].names.push(m.nickname);order[i].total+=m.power});const extra=bucket[4],extraTarget=groups[bucketIndex];extraTarget.names.push(extra.nickname);extraTarget.total+=extra.power}return Object.fromEntries(groups.map(g=>[g.code,g.names]))}
function autoAssign(){const team=currentTeam();if(team.members.length!==20){alert(`${selectedTeam} TEAM 최종 참전 멤버를 정확히 20명 선택해야 합니다.`);return}const sorted=team.members.map(n=>membersData.members.find(m=>m.nickname===n)).filter(Boolean).sort((a,b)=>b.power-a.power);if(sorted.length!==20){alert('멤버 데이터가 올바르지 않습니다. Member Manager를 확인하세요.');return}const r=assignRefineries(sorted),mh=assignMh(sorted);Object.assign(team.locations,r,mh,{CENTER:[0,1,4,5,8,9].map(i=>sorted[i].nickname)});const totals=code=>(team.locations[code]||[]).reduce((s,n)=>s+powerOf(n),0);$('#autoSummary').innerHTML=`<strong>${selectedTeam} TEAM 자동 배정 완료</strong><span>R1~R6 전투력 범위: ${Math.min(...['R1','R2','R3','R4','R5','R6'].map(totals)).toLocaleString()} ~ ${Math.max(...['R1','R2','R3','R4','R5','R6'].map(totals)).toLocaleString()}</span><span>M/H 각 5명 · CENTER 1·2·5·6·9·10위</span>`;renderLocationButtons();renderAssignments()}
...
async function saveAllGithub(){rememberToken();if(!memberSha){const m=await githubGetFile('data/members.json');memberSha=m.sha}if(!bgbSha){const b=await githubGetFile('data/bgb.json');bgbSha=b.sha}const date=todayKst(),mp=membersPayload(),bp=bgbPayload();if(!mp.lastUpdated)mp.lastUpdated=date;if(!bp.lastUpdated)bp.lastUpdated=date;TEAM_KEYS.forEach(t=>{if(bp.teams[t].members.length!==0&&bp.teams[t].members.length!==20)throw new Error(`${t} TEAM은 정확히 20명이어야 저장할 수 있습니다.`)});memberSha=await githubPutFile('data/members.json',mp,memberSha,`Update EZPK members ${mp.lastUpdated}`);bgbSha=await githubPutFile('data/bgb.json',bp,bgbSha,`Update EZPK BGB teams ${bp.lastUpdated}`);membersData=mp;bgbData=normalizeBgb(bp);syncInputs();setStatus('멤버 정보와 BGB A/B TEAM 편성 저장 완료. GitHub Pages 반영까지 보통 1~3분 정도 걸립니다.','ok')}

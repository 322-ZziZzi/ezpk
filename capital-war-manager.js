...
async function loadPublishedIntoDraft(){
  ...
  if(button){button.disabled=true;button.textContent='D1 명단 확인 중...';}
  try{
    // 관리자 권한 API에서 D1 원본을 직접 읽는다. 관리자 페이지에서는
    // 연맹원 전용 API보다 이 경로가 안정적이며 실제 저장 대상과 동일하다.
    let source=null;
    try{
      const adminData=await githubGetFile('data/capital-war.json');
      source=adminData?.data||null;
      if(adminData?.sha)sha=adminData.sha;
    }catch(adminError){
      console.warn('Admin Capital War D1 read failed; trying member endpoint.',adminError);
      const path='data/capital-war.json';
      const response=await fetch('/api/member/content?path='+encodeURIComponent(path)+'&restore='+Date.now(),{
        method:'GET',credentials:'include',cache:'no-store',headers:{accept:'application/json'}
      });
      const payload=await response.json().catch(()=>null);
      if(!response.ok)throw new Error(payload?.code||'CAPITAL_PUBLISHED_LOAD_FAILED');
      source=payload?.data?.content||payload?.content||payload;
    }

    // 현재 형식(published.teams)을 우선 사용하고, 오래된 저장본의 teams 및
    // 노출 당시 draft.teams 형식도 복구 대상으로 허용한다.
    const candidates=[source?.published?.teams,source?.teams,source?.draft?.teams];
    const rawTeams=candidates.find(teams=>teams&&TEAMS.some(key=>Array.isArray(teams[key])&&teams[key].length))||{};
    const rawCount=TEAMS.reduce((sum,key)=>sum+(Array.isArray(rawTeams[key])?rawTeams[key].length:0),0);
    if(!rawCount){
      alert('D1에서 현재 노출 중인 수도전 명단을 찾지 못했습니다. 공개 페이지를 강력 새로고침한 뒤에도 명단이 보이면 관리자에게 알려주세요.');
      return;
    }
    ...
    const warning=currentAssigned||cw.draft.participants.length
      ?`현재 편집 중인 목록을 D1의 노출 명단 ${publishedNames.length}명으로 교체할까요?`
      :`D1에 노출 중인 수도전 명단 ${publishedNames.length}명을 불러올까요?`;
    ...
    const suffix=unmatchedCount
      ?` 현재 회원 목록에서 찾지 못한 ${unmatchedCount}명도 노출 명단의 닉네임 그대로 복원했습니다.`
      :'';
    showAdminToast(`D1 노출 명단 ${publishedNames.length}명을 불러왔습니다. 저장 버튼을 눌러 편집본으로 확정하세요.${suffix}`);
  }catch(error){

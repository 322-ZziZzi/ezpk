-- EZPK v181
-- Add R5 as the administrator-only displayed rank and promote existing admin accounts.
...
CREATE TABLE members_v181 (
  ...
  member_rank TEXT NOT NULL
    CHECK (member_rank IN ('R1','R2','R3','R4','R5')),
...
SELECT
  id, login_id, password_hash, password_salt,
  password_algorithm, password_iterations,
  nickname, power, industry_level,
  CASE WHEN role = 'admin' THEN 'R5' ELSE member_rank END,
  role, status, must_change_password,
  nickname_updated_at, created_at, updated_at,
  last_login_at, password_changed_at
FROM members;
...
DROP TABLE members;
ALTER TABLE members_v181 RENAME TO members;
...
FROM members AS m
LEFT JOIN member_specs AS s ON s.member_id = m.id
...
VALUES ('member_db_schema_version', '3.1-v181', CURRENT_TIMESTAMP, 'migration')

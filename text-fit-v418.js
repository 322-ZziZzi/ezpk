(function(){
  ...
  const EXCLUDE = [
    ...
    '.board-title','.account-card','.mobile-account-profile'
  ].join(',');

  const SPECS = [
    {root:'.home-quick-grid', items:':scope > a', profile:'tile'},
    {root:'.home-account-actions', items:'button,a', profile:'tile'},
    {root:'[data-shared-header] .desktop-nav-items', items:'.nav-label', profile:'compact'},
    {root:'[data-shared-header] .header-account', items:':scope > .account-button', profile:'compact'},
    {root:'#mobileDrawerAccount .mobile-account-actions', items:'button', profile:'tile'},
    {root:'[data-shared-header] .account-menu', items:':scope > button', profile:'compact'},
    {root:'[data-shared-header]', items:'.alliance-selector-link', profile:'compact'},
    {root:'[data-shared-header] #navMoreMenu .nav-menu-group-items', items:'.nav-label', profile:'compact'},
    ...
    {root:'.game-library', items:'.game-choice h2', profile:'tile'},
    {root:'.game-library', items:'.game-choice a', profile:'compact'},
  ...
  function register(root, el, profile){
    if(!root || !el || isExcluded(el)) return;
  ...
  function registerKnown(scope=document){
    SPECS.forEach(spec=>{
      scope.querySelectorAll(spec.root).forEach(root=>{
        root.querySelectorAll(spec.items).forEach(el=>register(root,el,spec.profile));
      });
    });
    scope.querySelectorAll(TRANSLATED_INTERACTIVE).forEach(el=>{
      if(targetToGroup.has(el)||isExcluded(el)) return;
      const root=el.parentElement||el;
      register(root,el,'tile');
    });
  }
  ...
  function fitGroup(g){
    const targets=Array.from(g.targets).filter(el=>el.isConnected && !isExcluded(el));
  ...
  window.EZPKTextFit={refresh,schedule,version:'418'};
